...
def explain_detection(detection: dict) -> dict:
    ...
    source_filter = {"document_title": {"$in": allowed_sources}} if allowed_sources else None

    if source_filter:
        logger.debug("Module '%s' — restricting retrieval to %d sources", module, len(allowed_sources))
    else:
        logger.warning("Module '%s' has no source allowlist — searching all documents", module)
