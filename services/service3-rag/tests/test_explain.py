...
MOCK_EXPLANATION = {
    ...
    "citations": [{"source": "No Surprises Act at a Glance", "section": "p. 1", "url": None}],
}
...
@pytest.mark.parametrize("missing_field", ["error_id", "module", "error_type", "description"])
...
def test_explain_chain_not_initialized_returns_503(client):
    with patch("routes.explain.explain_detection", side_effect=RuntimeError("not initialized")):
        response = client.post("/explain", json={
            "session_id": "test-123",
            "errors": [VALID_ERROR],
        })
